...
for n = 1:numIterations
    ...
    grad_v = 2*transpose(U)*(YTrain-DTrain)/NTrain; % Gradient for the output layer
    grad_w = (2/NTrain)*(transpose(XTrain)*(((YTrain-DTrain)*Vout').*(1-U.^2))); % And the input layer
    grad_w = grad_w(:,1:(size(grad_w,2)-1));
